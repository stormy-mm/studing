...
class ListViewTest(TestCase):
    ...
    def post_invalid_input(self):
        ...
        return self.client.post(
            f"/lists/{self.list_.id}/",
            data={'text': ''}
        )

    def test_uses_list_template(self):
        """Тест: используется шаблон списка"""
        response = self.client.get(f"/lists/{self.list_.id}/")
        self.assertTemplateUsed(response, "list.html")

    def test_displays_all_list_items(self):
        """Тест: отображаются все элементы конкретного списка"""
    ...
    def test_passes_list_template(self):
        ...
        response = self.client.get(f"/lists/{self.list_.id}/")
        self.assertEqual(response.context["list"], self.list_)
    ...
    def test_duplicate_item_validation_errors_end_up_on_lists_page(self):
        ...
        Item.objects.create(list=self.list_, text="textey")
        response = self.client.post(
            f"/lists/{self.list_.id}/",
            data={"text": "textey"}
        )
